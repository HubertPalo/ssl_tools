...
usage() {
    ...
    echo "  --load_backbone: Path to the backbone model checkpoint to load"
    echo "  <SCRIPT_ARGS>: Additional arguments to pass to the script"

    echo ""

    echo "Example:"
    echo "  $0 --log_dir logs --root_dataset_dir datasets --train_dataset KuHar --script tnc_head_classifier.py --name tnc_head --cwd .. --input_size 180 --num_classes 6 --transforms fft --load_backbone /path/to/backbone"
}
...
# Parses the update_backbone parameter and removes it from the script arguments
if [[ " ${SCRIPT_ARGS[@]} " =~ " --update_backbone " ]]; then
    UPDATE_BACKBONE="True"
    SCRIPT_ARGS=("${SCRIPT_ARGS[@]/"--update_backbone"/}")
fi
...
write_and_run_test() {
    ...
    local checkpoint_path="$LOG_DIR/finetune/$NAME/$RUN_ID/checkpoints/last.ckpt"
    local config_file="$output_dir/config.yaml"
